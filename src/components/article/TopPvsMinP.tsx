...
export default function TopPvsMinP() {
	...
	return (
		<div style={css.wrap}>
			...
			<div style={css.note}>
				<strong>top_p</strong> считает накопленную массу: даже когда лидер держит 90%, порог 0.95 одним токеном
				не наберётся — «дособрать» массу приходится из хвоста, и это его слабое место. На размазанном
				распределении та же логика пропускает почти всё. <strong>min_p</strong> считает порог от лидера,
				поэтому на уверенном режет жёстко, а на размазанном отпускает. Одно значение — разное поведение, и в
				этом весь смысл.
			</div>
		</div>
	);
