	...
	return (
		<div style={css.wrap}>
			...
			<div style={css.desc}>
				...
				ручки, только собранные в один конвейер, чтобы увидеть всю форму сразу. Размер словаря и числа кандидатов ниже —
				иллюстративные, не измерены на реальном форвард-пассе: точные значения зависят от
				логитов конкретной модели на конкретном шаге, которых у нас нет. Порядок ручек и характер их эффекта —
				настоящие.
			</div>
			<div style={css.order}>порядок как в vLLM: словарь → temperature → min_p → top_k → top_p → финальная softmax → 1 токен</div>
